"""Structured-ish logging — copy of sde_importer/log.py.

Kept intentionally duplicated rather than shared via a sibling package:
the two one-shot containers ship as separate images with separate
requirements files, and we don't want to introduce a third "common"
package just to dedupe ~100 lines of stdlib-logging glue. If a third
tool ever needs the same wrapper, promote this to `python/_common/`.
"""
