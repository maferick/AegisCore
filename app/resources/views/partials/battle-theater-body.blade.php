...
@php
    ...
    // Maps keyed on internal side key (SIDE_A/B/C) — used by the kill
    // feed + pilot list for row-start chips. Built once from $sideList
    // so both layers use the same palette post-promotion.
    $sideColorByKey = [];
    $sideLabelByKey = [];
    $sideIndexByKey = [];
    foreach ($sideList as $_idx => $_s) {
        $sideColorByKey[$_s['key']] = $_s['color'];
        $sideLabelByKey[$_s['key']] = $_s['label_override']
            ?? ($_s['key'] === S::SIDE_C ? 'Third parties' : 'Side '.$_s['key']);
        $sideIndexByKey[$_s['key']] = $_idx;
    }
@endphp
...
<div class="km-card" style="margin-bottom: 1.5rem;">
    ...
    @foreach ($kill_feed as $km)
        @php
            ...
            $vSideLabel  = $sideLabelByKey[$vSide] ?? '';
            $isHighValue = $km['total_value'] >= 1_000_000_000;
            $highClass   = $isHighValue ? 'bt-kill-high' : '';
        @endphp
        <div class="km-attacker {{ $highClass }}"
             style="border-left: 3px solid {{ $vSideColor }}; padding-left: 0.6rem;">
            <span title="{{ $vSideLabel }}" style="display:inline-block;width:6px;height:32px;background:{{ $vSideColor }};border-radius:2px;margin-right:6px;flex-shrink:0;"></span>
