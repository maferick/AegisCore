#!/usr/bin/env bash
# Spec 5 v0 diagnostic batch runner. Runs battle_role_scoring against
# the 8 validation battles and logs to /tmp/spec5_batch.log.
set -u
cd "$(git rev-parse --show-toplevel 2>/dev/null || echo /opt/AegisCore)"
LOG=/tmp/spec5_batch.log
: > "$LOG"
...
for case in "${CASES[@]}"; do
    ...
    docker compose --env-file .env -f infra/docker-compose.yml --profile tools run --rm battle_role_scoring \
        run --battle-id "$bid" --alliance-id "$aid" 2>&1 | tail -20 >> "$LOG"
done
